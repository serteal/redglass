...
from redglass.data import Dialogue
from redglass.data.base import DialogueDataset, TargetedDialogueDataset
from redglass.log import logger
from redglass.models import ModelType
from redglass.types import Message
...
class TokenizedDataset:
    """
    A class to represent tokenized input data for activation-based detectors. If the dataset
    contains a pair of safe/unsafe dialogues, this class only represents ONE of those.
        ...
        for batch_idx, dialogue in enumerate(dialogues):
            ...
            for message in dialogue:
                ...
                if message.detect:
                    ...
                    detection_mask[
                        batch_idx,
                        start_tok_index - padding["left"] : end_tok_index
                        + padding["right"],
                    ] = True
    ...
    def from_dialogue_list(
        ...
        detection_mask_padding: dict[str, dict[str, int]],
        ...
        Args:
            ...
            detection_mask_padding (dict[str, dict[str, int]]): Padding configuration for detection mask. Non-zero
        ...
        if tokenizer.name_or_path.startswith("google/gemma"):
            ...
            raise NotImplementedError("Only gemma and llama tokenizers are supported")
        ...
        fold_system = tokenizer_type == "gemma"
        ...
        detection_mask = cls._get_detection_mask(
            ...
            for_generation,
            detect_all,
            detection_mask_padding[tokenizer_type],
            cast(ModelType, tokenizer_type),
        )
    ...
    def from_dataset(
        ...
        """
        Create a TokenizedDataset from a DialogueDataset or TargetedDialogueDataset

        Args:
            dataset (DialogueDataset | TargetedDialogueDataset): The dataset containing dialogues to tokenize.
            tokenizer (PreTrainedTokenizerBase): Tokenizer to use for processing the text.
        ...
        Returns:
            TokenizedDataset: A new instance of TokenizedDataset.
        """
    ...
    def __getitem__(self, idx: int | slice) -> "TokenizedDataset":
...
def get_tokenized_dataset_pair(
    safe: DialogueDataset | TargetedDialogueDataset,
    unsafe: DialogueDataset | TargetedDialogueDataset,
    tokenizer: PreTrainedTokenizerBase,
    ...
    Args:
        safe (DialogueDataset | TargetedDialogueDataset): Dataset containing safe dialogues.
        unsafe (DialogueDataset | TargetedDialogueDataset): Dataset containing unsafe dialogues.
        tokenizer (PreTrainedTokenizerBase): Tokenizer to use for processing the text.
    ...
    safe_tokenized = TokenizedDataset.from_dataset(
        safe,
        tokenizer,
        detect_all=detect_all,
        detect_only_start_of_turn=detect_only_start_of_turn,
    ...
    unsafe_tokenized = TokenizedDataset.from_dataset(
        unsafe,
        tokenizer,
        detect_all=detect_all,
        detect_only_start_of_turn=detect_only_start_of_turn,
        detect_only_last_token=detect_only_last_token,
        additional_id="unsafe",
    )

    if enforce_equal_detect_lengths:
        equalize_detection_masks(safe_tokenized, unsafe_tokenized)

    return safe_tokenized, unsafe_tokenized
