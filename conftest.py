...
from redglass.models import ModelName, get_model_and_tokenizer
...
def llama_model() -> PreTrainedModel:
    model, _ = get_model_and_tokenizer(ModelName.LLAMA_8B)
    assert model is not None
    return model
...
def llama_tokenizer() -> PreTrainedTokenizerBase:
    return get_model_and_tokenizer(ModelName.LLAMA_8B, omit_model=True)[1]
