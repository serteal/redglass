...
def test_auroc_random_data():
    ...
    honest_scores = torch.tensor(
        np.random.normal(loc=0.3, scale=0.2, size=n_samples).clip(0, 1)
    )
    deceptive_scores = torch.tensor(
        np.random.normal(loc=0.7, scale=0.2, size=n_samples).clip(0, 1)
    )
...
def test_get_tpr_at_fpr_from_paired_dataset():
    ...
    tpr_at_fpr_01 = get_tpr_at_fpr_from_paired_dataset(
        honest_scores, deceptive_scores, 0.1
    )
    tpr_at_fpr_05 = get_tpr_at_fpr_from_paired_dataset(
        honest_scores, deceptive_scores, 0.5
    )
