...
from redglass.repository import ALL_DATASETS_DICT
...
def test_load_all_datasets(dataset_id: str):
    ...
    assert isinstance(dataset, DialogueDataset | TargetedDialogueDataset)
    ...
    if dataset.metadata is not None:
        ...
        for k, v in dataset.metadata.items():
            ...
            assert len(v) == len(dataset.dialogues), (
                f"Metadata {k} has incorrect length"
            )
...
def test_add_datasets(dataset_class: type[DialogueDataset | TargetedDialogueDataset]):
    dataset = dataset_class()

    # Test adding dataset to itself
    add_to_self = dataset + dataset
    assert isinstance(add_to_self, DialogueDataset | TargetedDialogueDataset)
    assert len(add_to_self) == len(dataset) * 2

    # Test adding dataset to another dataset
    other_dataset = next(
        (d() for d in ALL_DATASETS_DICT.values() if d != dataset_class), None
    )
    if other_dataset:
        add_to_other = dataset + other_dataset
        assert isinstance(add_to_other, DialogueDataset | TargetedDialogueDataset)
        assert len(add_to_other) == len(dataset) + len(other_dataset)
        assert (
            add_to_other.base_name == f"{dataset.base_name}+{other_dataset.base_name}"
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # (WerewolfDataset, {"role", "night_info", "discussion"}),
        # (RolePlayingDataset, {"scenario", "question", "answer_prefix"}),
    ],
)
def test_metadata(
    dataset_class: type[DialogueDataset | TargetedDialogueDataset],
