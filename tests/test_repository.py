...
from redglass.repository import (
    ALL_DATASETS,
    ALL_DATASETS_DICT,
    DatasetRepository,
    get_id,
)
from redglass.types import Dialogue, DialogueDataType, Label, Message
...
def test_get_prewritten_dataset(dataset_repository: DatasetRepository):
    ...
    dataset = dataset_repository.get("circuit_breakers__plain")
    assert dataset is not None
    assert isinstance(dataset, TargetedDialogueDataset)
    assert dataset.base_name == "circuit_breakers"
    assert dataset.variant == "plain"
...
def test_add_and_get_rollout_dataset(
    dataset_repository: DatasetRepository, tmp_path: Path
):
...
def test_check_dataset_name(dataset_repository: DatasetRepository):
    ...
    assert (
        dataset_repository.check_dataset_name("circuit_breakers__plain")
        == "circuit_breakers__plain"
    )
...
def test_rollout_dataset_trim_reasoning(
    dataset_repository: DatasetRepository, tmp_path: Path
):
    ...
    class TestDataset(DialogueDataset):
        ...
        def _get_dialogues(self) -> DialogueDataType:
            ...
            return dialogues, [Label.UNSAFE], None
