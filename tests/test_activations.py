...
from redglass.data import Dialogue, DialogueDataset
from redglass.data.tokenized_data import TokenizedDataset
from redglass.types import Label, Message
...
def test_activations(
    ...
    activations = Activations.from_model(
        gemma_model, tokenized_dataset, save_logits=True
    )
    ...
    assert activations.get_masked_activations().shape == (
        n_detect_toks,
        n_layers + 1,
        d_emb,
    )
...
def test_save_load_activations(
    ...
    assert torch.equal(
        activations.get_masked_activations(),
        loaded_activations.get_masked_activations(),
    )
    assert torch.equal(
        activations.get_masked_logits(), loaded_activations.get_masked_logits()
    )
...
def paired_dataset():
    ...
    honest_dataset = DialogueDataset(
        "plain", honest_dialogues, labels=[Label.SAFE] * len(honest_dialogues)
    )
    deceptive_dataset = DialogueDataset(
        "plain",
        deceptive_dialogues,
        labels=[Label.UNSAFE] * len(deceptive_dialogues),
    )
...
def test_split_paired_activations(
    ...
    assert len(split_acts.safe_acts) == len(paired_dataset.get_safe())
    assert len(split_acts.unsafe_acts) == len(paired_dataset.get_unsafe())

    paired_dataset.padding = {"gemma": {"left": 2, "right": 0}}

    # Test detect_k_prior_tokens
    split_acts_k2 = PairedActivations.from_dataset(
        paired_dataset, gemma_tokenizer, gemma_model
    )

    assert split_acts_k2.safe_acts.tokenized_dataset.detection_mask is not None
    assert split_acts.safe_acts.tokenized_dataset.detection_mask is not None

    assert (
        split_acts_k2.safe_acts.tokenized_dataset.detection_mask.sum()
        < split_acts.safe_acts.tokenized_dataset.detection_mask.sum()
    )
...
def test_split_paired_detect_only_start_of_turn(
    ...
    num_safe = len(paired_dataset.get_safe())
    assert split_acts_start.safe_acts.tokenized_dataset.detection_mask is not None
    assert split_acts_start.safe_acts.tokenized_dataset.detection_mask.sum() == num_safe
    ...
    masked_toks = split_acts_start.safe_acts.tokenized_dataset.tokens[
        split_acts_start.safe_acts.tokenized_dataset.detection_mask
    ]
...
def test_split_paired_layers(
    ...
    safe_acts, unsafe_acts = split_acts.get_all_acts()
    assert safe_acts.layers == unsafe_acts.layers == layers
