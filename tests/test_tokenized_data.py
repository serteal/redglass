...
def get_mask_text(
    tokenized: TokenizedDataset, tokenizer: PreTrainedTokenizerBase
) -> str:
    assert len(tokenized) == 1
    return decode(tokenized.tokens[tokenized.detection_mask], tokenizer)


@pytest.mark.parametrize("tokenizer_fixture", ["gemma_tokenizer", "llama_tokenizer"])
def test_tokenized_dialogue(
    ...
    short_dialogue = [
        ...
        Message("assistant", "Hi!", True),
    ]

    tokenized = TokenizedDataset.from_dialogue_list(
        [long_dialogue, short_dialogue],
        tokenizer,
        padding={model: {"left": 0, "right": 0} for model in ["gemma", "llama"]},
    )
